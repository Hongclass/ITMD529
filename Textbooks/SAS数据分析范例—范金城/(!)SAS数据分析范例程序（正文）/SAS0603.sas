...
%let k=1;
  ...
  %do %while(&n ne);
%do least=1 %to 2;
title1 " &&d&i fastclus maxc=&n least=&least";
...
%plotit(data=prelim, plotvars=&&b&i &&a&i,
        labelvar=preclus, typevar=preclus,colors=white,font=Italic);
title1 "fastclus maxc=&n least=&least labelvar=d";
%plotit(data=prelim, plotvars=&&b&i &&a&i,
        labelvar=d, typevar=preclus,colors=white,font=Italic);
run;
...
%let plotitop=gopts=cback=ligr, color=black;
%plotit(data=outseeds&i, plotvars=&&b&i &&a&i,
        labelvar=preclus, typevar=preclus,colors=black);
title "outseeds maxc=&n least=&least labelvar=_near_";
%plotit(data=outseeds&i, plotvars=&&b&i &&a&i,
        labelvar=_near_, typevar=preclus,colors=black);
title "outseeds maxc=&n least=&least labelvar=_freq_";
%plotit(data=outseeds&i, plotvars=&&b&i &&a&i,
        labelvar=_freq_, typevar=preclus,black);
run;
%end;
%let k=%eval(&k+1);
  %let n=%scan(&maxc,&k);
