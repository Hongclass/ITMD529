...
proc gplot data=f&i;
symbol i=join v=dot;
plot u1*date/cframe=cyan;
plot u2*date/cframe=cyan;
...
proc gplot data=g&i;
plot u1*date=_type_/cframe=yellow;
plot u2*date=_type_/cframe=yellow;
where _type_='RESIDUAL';
symbol i=needle c=blue;
run;
proc gplot data=g&i;
plot u1*date=_type_/cframe=pink;
plot u2*date=_type_/cframe=pink;
