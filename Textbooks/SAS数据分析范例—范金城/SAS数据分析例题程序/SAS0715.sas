...
%macro discrim(pool,priors);
proc candisc data=&&d&i out=b&i;
class c;
var  &&c&i;
run;
title "discrim &&c&i pool=&pool priors=&priors";
proc discrim data=b&i method=normal pool=&pool out=a1 outstat=a2 outcross=a3
...
title1 "&&d&i a1  pool=&pool priors=&priors typevar=c";
%plotit(data=a1, plotvars=date x2,
        labelvar=date, typevar=c);
title1 "&&d&i a1  pool=&pool priors=&priors typevar=_INTO_";
%plotit(data=a1, plotvars=date x2,
        labelvar=date, typevar=_INTO_);
title1 "&&d&i a3  pool=&pool priors=&priors typevar=_INTO_";
%plotit(data=a3, plotvars=date x2,
        labelvar=date, typevar=_INTO_);
title "discrim &&d&i normal pool=&pool priors=&priors";
%plot;
proc sort data=a1;
by c;
run;
proc print data=a1(keep=date c _into_);
title "print data=a1";
run;
proc sort data=a3;
by c;
run;
proc print data=a3(keep=date c _into_);
title "print data=a3";
run;
%mend;
