...
CLASS tran_code / PRELOADFMT;
table (tran_code All) All, sum='All'*hh='HHs'*f=comma12. (dda='Checking' mms='Mon Mkt' sav='Savings' tda='Time Dep' ira='IRA' mtg='Mortgage' heq='Home Eq' card='Credit Card' iln='Dir Loan' ind='Ind Loan' sln='Student loan' sec='Securities' ins='Insurance'
...
table (tran_code All) All, sum='All'*hh='HHs'*f=comma12. (dda_amt='Checking'*pctsum<dda>='Avg. Bal.' mms_amt='Mon Mkt'*pctsum<mms>='Avg. Bal.' sav_amt='Savings'*pctsum<sav>='Avg. Bal.' tda_amt='Time Dep'*pctsum<tda>='Avg. Bal.'
...
CLASS tran_code / PRELOADFMT;
table (tran_code All) All, sum='All'*hh='HHs'*f=comma12. (dda_con='Checking'*pctsum<hh>='Avg. Con.' mms_con='Mon Mkt'*pctsum<hh>='Avg. Con.' sav_con='Savings'*pctsum<hh>='Avg. Con.' tda_con='Time Dep'*pctsum<hh>='Avg. Con.' ira_con='IRA'*pctsum<hh>='Avg.
...
where tran_code ne '';
var hh;
CLASS tran_code segment band ixi_tot cbr distance tenure_yr tran_code/ PRELOADFMT;
table (tran_code All) All, sum='All'*hh='HHs'*f=comma12. (segment)*N='HHs'*f=comma12. segment*rowPCTN="Percent"*f=pctfmt./ nocellmerge misstext ='0';
table (tran_code All) All, sum='All'*hh='HHs'*f=comma12. (tran_code='Tran. Segment')*N='HHs'*f=comma12. tran_code='Tran. Segment'*rowPCTN="Percent"*f=pctfmt./ nocellmerge misstext ='0';
table (tran_code All) All, sum='All'*hh='HHs'*f=comma12. (ixi_tot='Estimated Wealth')*N='HHs'*f=comma12. ixi_tot='Estimated Wealth'*rowPCTN="Percent"*f=pctfmt./ nocellmerge misstext ='0';
table (tran_code All) All, sum='All'*hh='HHs'*f=comma12. (cbr)*N='HHs'*f=comma12. cbr*rowPCTN="Percent"*f=pctfmt./ nocellmerge misstext ='0';
table (tran_code All) All, sum='All'*hh='HHs'*f=comma12. (distance='Dist to Branch')*N='HHs'*f=comma12. distance='Dist to Branch'*rowPCTN="Percent"*f=pctfmt./ nocellmerge misstext ='0';
table (tran_code All) All, sum='All'*hh='HHs'*f=comma12. (tenure_yr="tenure (Yrs)")*N='HHs'*f=comma12. tenure_yr="tenure (Yrs)"*rowPCTN="Percent"*f=pctfmt./ nocellmerge misstext ='0';
table (tran_code All) All, sum='All'*hh='HHs'*f=comma12. (band='Profit Band')*N='HHs'*f=comma12. band='Profit Band'*rowPCTN="Percent"*f=pctfmt./ nocellmerge misstext ='0';
format tran_code $transegm. ;
...
set data.main_201303;
where tran_code ne '';
keep tran_code dda: mms: sav: tda: ira: mtg: iln: ind: heq: card: ccs: sec: hhid ;
...
balance = heq_amt;
output;
keep order balance tran_code hhid;
run;
proc format;
...
data box1;
set box1;
variable = put(tran_code,$transegm.);
run;
proc sql;
select count(unique(variable)) into :class_n TRIMMED from box1 ;
quit;
proc sql;
select variable, count(*) into :name1 - :name8, :n1 - :n8 from box1 group by variable ;
quit;
proc sort data=box1;
by variable;
run;
...
set data.main_201303;
where tran_code ne '';
keep tran_code dda mms sav tda ira mtg iln ind heq card sec hhid ;
run;
data cbox;
...
output;
keep order contribution tran_code hhid;
run;
proc format;
value order 1 = 'Checking' 2 = 'Money Mkt' 3 = 'Savings' 4 = 'Time Dep' 5 = 'IRAs' 6 = 'Credit Card' 7 = 'Dir. Loan' 8 = 'Ind. Loan' 9 = 'Securities' 10= 'Mortgage' 11='Home Equity';
run;
...
quit;
proc sql;
select variable, count(*) into :name1 - :name8, :n1 - :n8 from cbox1 group by variable ;
quit;
...
ods pdf file="C:\Documents and Settings\ewnym5s\My Documents\Virtually Domiciled\tran_segm_profile_201303b.pdf" style=mtbnew nogfootnote dpi=300;
...
boxplot x=order y=amt1 / discreteoffset=-0.3 boxwidth=.1 outlierattrs=(color=grey) medianattrs=(color=red) meanattrs=(color=red symbol=DiamondFilled) display=(CAPS FILL MEAN MEDIAN) name="1" legendlabel="ATM Dominant";
boxplot x=order y=amt2 / discreteoffset=-0.2 boxwidth=.1 outlierattrs=(color=grey) medianattrs=(color=red) meanattrs=(color=red symbol=DiamondFilled) display=(CAPS FILL MEAN MEDIAN) name="2" legendlabel="Branch Dominant";
boxplot x=order y=amt3 / discreteoffset=-0.1 boxwidth=.1 outlierattrs=(color=grey) medianattrs=(color=red) meanattrs=(color=red symbol=DiamondFilled) display=(CAPS FILL MEAN MEDIAN) name="3" legendlabel="Inactive";
boxplot x=order y=amt4 / discreteoffset=0 boxwidth=.1 outlierattrs=(color=grey) medianattrs=(color=red) meanattrs=(color=red symbol=DiamondFilled) display=(CAPS FILL MEAN MEDIAN) name="4" legendlabel="Multi - High Branch";
boxplot x=order y=amt5 / discreteoffset=0.1 boxwidth=.1 outlierattrs=(color=grey) medianattrs=(color=red) meanattrs=(color=red symbol=DiamondFilled) display=(CAPS FILL MEAN MEDIAN) name="5" legendlabel="Multi - Low Branch";
boxplot x=order y=amt6 / discreteoffset=0.2 boxwidth=.1 outlierattrs=(color=grey) medianattrs=(color=red) meanattrs=(color=red symbol=DiamondFilled) display=(CAPS FILL MEAN MEDIAN) name="6" legendlabel="Multi - Med Branch";
boxplot x=order y=amt7 / discreteoffset=0.3 boxwidth=.1 outlierattrs=(color=grey) medianattrs=(color=red) meanattrs=(color=red symbol=DiamondFilled) display=(CAPS FILL MEAN MEDIAN) name="7" legendlabel="Online Dominant";
boxplot x=order y=amt8 / discreteoffset=0.4 boxwidth=.1 outlierattrs=(color=grey) medianattrs=(color=red) meanattrs=(color=red symbol=DiamondFilled) display=(CAPS FILL MEAN MEDIAN) name="8" legendlabel="Phone Dominant";
referenceline y=0 / lineattrs=(pattern=dot);
discretelegend "1" "2" "3" "4" "5" "6" "7" "8" / location=outside valign=bottom halign=center across=3 DISPLAYCLIPPED=Yes;
endlayout;
endgraph;
end;
run;
...
layout overlay / yaxisopts=(linearopts=( viewmax =237500 viewmin=0 tickvalueformat=dollar12.) label='Balance' labelattrs=(weight=bold)) xaxisopts=( label="Product" labelattrs=(weight=bold) discreteopts=(TICKVALUEFITPOLICY=STAGGER) ) cycleattrs=true ;
boxplot x=order y=amt1 / discreteoffset=-0.3 boxwidth=.1 outlierattrs=(color=grey) medianattrs=(color=red) meanattrs=(color=red symbol=DiamondFilled) display=(CAPS FILL MEAN MEDIAN) name="1" legendlabel="ATM Dominant";
boxplot x=order y=amt2 / discreteoffset=-0.2 boxwidth=.1 outlierattrs=(color=grey) medianattrs=(color=red) meanattrs=(color=red symbol=DiamondFilled) display=(CAPS FILL MEAN MEDIAN) name="2" legendlabel="Branch Dominant";
boxplot x=order y=amt3 / discreteoffset=-0.1 boxwidth=.1 outlierattrs=(color=grey) medianattrs=(color=red) meanattrs=(color=red symbol=DiamondFilled) display=(CAPS FILL MEAN MEDIAN) name="3" legendlabel="Inactive";
boxplot x=order y=amt4 / discreteoffset=0 boxwidth=.1 outlierattrs=(color=grey) medianattrs=(color=red) meanattrs=(color=red symbol=DiamondFilled) display=(CAPS FILL MEAN MEDIAN) name="4" legendlabel="Multi - High Branch";
...
vbar tran_code / missing group=proxy groupdisplay=cluster response=col1 nostatlabel grouporder=data datalabel DATALABELATTRS=(Size=6);
...
data segments1;
set segments1;
if _table_ eq 2 and hh_sum ne . then pct = hh_sum/ 1193088;
run;
...
class tran_code;
var hh;
table tran_code='Analysis Group' ALL='Total',N='HHs'*f=comma12. pctN='Percent'*f=pctfmt./ nocellmerge ;
format tran_code $transegm.;
run;
ods pdf close;
