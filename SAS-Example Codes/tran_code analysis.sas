...
keep hhid segment cbr market band rm hh dda mms sav tda ira mtg heq iln ind card sln sec ins trs sdb dda_amt mms_amt sav_amt tda_amt ira_amt mtg_amt heq_amt iln_amt ind_amt ccs_amt sln_amt sec_amt trs_amt tran_code;
rename card=ccs;
run;
data temp_contrib;
...
table (tran_code ALL) , (hh dda mms sav tda ira mtg heq iln ind ccs sln sec ins trs sdb)*(sum) (dda_amt mms_amt sav_amt tda_amt ira_amt mtg_amt heq_amt iln_amt ind_amt ccs_amt sln_amt sec_amt trs_amt)*(sum) (dda_amt mms_amt sav_amt tda_amt ira_amt mtg_amt
...
con_tot_hh = 0;
output;
keep tran_code product prod_hh penetration balance bal_prod_hh bal_tot_hh contribution con_prod_hh con_tot_hh;
...
if cbr eq . then cbr = 99;
if market eq . then market = 99;
keep tran_code cbr market segment band tenure_yr rm svcs cqi: hh hhid;
run;
proc tabulate data=temp_data out=segment_results missing;
class tran_code ;
class segment;
var hh;
table (tran_code ALL) *(segment ALL), hh*sum*f=comma12. / nocellmerge;
...
class cbr;
var hh;
table (tran_code ALL) *(cbr ALL), hh*sum*f=comma12. / nocellmerge;
...
class market;
var hh;
table (tran_code ALL) *(market ALL), hh*sum*f=comma12. / nocellmerge;
format market mkt2012fmt. ;
run;
data market_results;
set market_results;
if substr(_type_,1,1) eq 0 then do;
...
class band;
var hh;
table (tran_code ALL) *(band ALL), hh*sum*f=comma12. / nocellmerge;
format band $2. ;
run;
data band_results;
set band_results;
...
class tenure_yr;
var hh;
table (tran_code ALL) *(tenure_yr ALL), hh*sum*f=comma12. / nocellmerge;
format tenure_yr tenureband. ;
run;
...
class svcs;
var hh;
table (tran_code ALL) *(svcs ALL), hh*sum*f=comma12. / nocellmerge;
format svcs comma2. ;
run;
data svcs_results;
set svcs_results;
...
class cqi;
var hh;
table (tran_code ALL) *(cqi ALL), hh*sum*f=comma12. / nocellmerge;
format cqi cqifmt. ;
run;
data cqi_results;
set cqi_results;
...
class cqi_dd;
var hh;
table (tran_code ALL) *(cqi_dd ALL), hh*sum*f=comma12. / nocellmerge;
format cqi_dd binary_flag. ;
run;
data cqi_dd_results;
set cqi_dd_results;
if substr(_type_,1,1) eq 0 then do;
...
class cqi_bp;
var hh;
table (tran_code ALL) *(cqi_bp ALL), hh*sum*f=comma12. / nocellmerge;
format cqi_bp binary_flag. ;
run;
data cqi_bp_results;
set cqi_bp_results;
if substr(_type_,1,1) eq 0 then do;
...
class cqi_web;
var hh;
table (tran_code ALL) *(cqi_web ALL), hh*sum*f=comma12. / nocellmerge;
format cqi_web binary_flag. ;
run;
...
class cqi_deb;
var hh;
table (tran_code ALL) *(cqi_deb ALL), hh*sum*f=comma12. / nocellmerge;
format cqi_deb binary_flag. ;
run;
...
class cqi_odl;
var hh;
table (tran_code ALL) *(cqi_odl ALL), hh*sum*f=comma12. / nocellmerge;
format cqi_odl binary_flag. ;
run;
...
class rm;
var hh;
table (tran_code ALL) *(rm ALL), hh*sum*f=comma12. / nocellmerge;
format rm $rmfmt. ;
run;
data rm_results;
set rm_results;
...
edeliv_num = 0;
estat_num = 0;
keep tran_code HHID HH web bp WAP SMS edeliv estat fico FWorks web_signon BP_NUM BP_AMT SMS_NUM WAP_NUM fico_num fworks_num web1 bp1 sms1 wap1 fico1 fworks1 edeliv1 estat1 estat_num edeliv_num;
...
table (tran_code ALL), (sum)*(HH web bp WAP SMS edeliv estat fico FWorks web_num BP_NUM BP_AMT SMS_NUM WAP_NUM fico_num fworks_num web1 bp1 sms1 wap1 fico1 fworks1 edeliv1 estat1 estat_num edeliv_num) / nocellmerge;
run;
data web2;
set web1;
if substr(_type_,1,1) eq 0 then do;
...
keep tran_code service enrolled: active: volume_avg spend_avg;
run;
*************************************************;
...
keep tran_code HHID HH web web1 bp1 bp web_signon BP_NUM BP_AMT vpos: mpos: br_tr: vru: chk: dd: atmo: atmt: cqi_dd ;
...
table (tran_code ALL), (sum)*(HH vpos: mpos: chk: atmo: atmt: dd: bp: ) / nocellmerge;
run;
data tran2;
set tran1;
if substr(_type_,1,1) eq 0 then do;
...
keep tran_code Transaction Enrolled: active: volume_avg spend_avg;
run;
******************************************;
...
set data.main_201212;
where dda eq 1;
keep tran_code HHID HH clv: ixi: ;
run;
...
table (tran_code ALL), N (clv_total clv_rem clv_rem_ten)*mean;
run;
data clv2;
set clv1;
if substr(_type_,1,1) eq 0 then do;
...
where clv_total ne .;
class tran_code clv_total;
var HH ;
table (tran_code ALL) ,(clv_total ALL)*(hh)*(sum rowpctsum<hh>) ;
...
format ;
keep tran_code clv_total HHs percent_hhs;
run;
proc tabulate data=temp_clv out=ixi1;
where ixi_tot ne .;
class tran_code ;
var HH ixi: ;
table (tran_code ALL), N (ixi:)*mean;
run;
data ixi2;
set ixi1;
if substr(_type_,1,1) eq 0 then do;
...
where ixi_tot ne .;
class tran_code IXI_tot;
var HH ;
table (tran_code ALL) ,(ixi_tot ALL)*(hh)*(sum rowpctsum<hh>*f=pctfmt.) ;
...
percent_hhs = sum (of hh_pctsum:);
format ;
keep tran_code ixi_tot HHs percent_hhs;
run;
ods html;
