...
run;
proc tabulate data=data.main_201303(obs=10000) order=data out=penet missing;
...
run;
data temp_main;
set data.main_201303(obs=10000);
where dda eq 1 and mma_quintile ne .;
...
format mma_quintile quintiles. ;
run;
proc tabulate data=data.main_201303(obs=10000) order=data out=segments missing;
where dda eq 1 and mma_quintile ne .;
var hh;
...
ods pdf file="C:\Documents and Settings\ewnym5s\My Documents\Analysis\Virtually Domiciled\mma_prospects_1.pdf" style=mtbnew nogfootnote dpi=300;
...
layout overlay / yaxisopts=(linearopts=( viewmax =887500 viewmin=0 tickvalueformat=dollar12.) label='Balance' labelattrs=(weight=bold)) xaxisopts=( label="Product" labelattrs=(weight=bold) discreteopts=(TICKVALUEFITPOLICY=STAGGER) ) cycleattrs=true ;
