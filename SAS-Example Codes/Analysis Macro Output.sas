...
where tran_segment ne '';
keep hhid segment cbr market band rm hh dda mms sav tda ira mtg heq iln ind card sln sec ins trs sdb dda_amt mms_amt sav_amt tda_amt ira_amt mtg_amt heq_amt iln_amt ind_amt ccs_amt sln_amt sec_amt trs_amt tran_segment segment;
rename card=ccs;
run;
data temp_contrib;
set data.contrib_201203;
...
table (tran_segment ALL)*(segment ALL) , (hh dda mms sav tda ira mtg heq iln ind ccs sln sec ins trs sdb)*(sum) (dda_amt mms_amt sav_amt tda_amt ira_amt mtg_amt heq_amt iln_amt ind_amt ccs_amt sln_amt sec_amt trs_amt)*(sum) (dda_amt mms_amt sav_amt
tda_amt ira_amt mtg_amt heq_amt iln_amt ind_amt ccs_amt sln_amt sec_amt trs_amt)*(mean) (DDA_CON MMS_CON SAV_CON TDA_CON IRA_CON SEC_CON TRS_CON mtg_con heq_con ccs_con ILN_CON SLN_CON ind_con)*sum (DDA_CON MMS_CON SAV_CON TDA_CON IRA_CON SEC_CON TRS_CON
mtg_con heq_con ccs_con ILN_CON SLN_CON ind_con)*mean;
run;
data bal2;
set bal1;
...
con_tot_hh = 0;
output;
keep tran_segment segment product prod_hh penetration balance bal_prod_hh bal_tot_hh contribution con_prod_hh con_tot_hh;
...
where tran_segment ne '';
cqi = sum(of cqi:);
keep tran_segment segment cbr market segment band tenure_yr rm svcs cqi: hh hhid;
run;
proc tabulate data=temp_data out=cbr_results missing;
class tran_segment segment cbr;
var hh;
table (tran_segment ALL)*(segment ALL) * (cbr ALL), hh*sum*f=comma12. / nocellmerge;
...
if substr(_type_,3,1) eq 0 then do;
cbr = -1;
end;
format segment segfmt.;
proc tabulate data=temp_data out=market_results missing;
class tran_segment segment market;
var hh;
table (tran_segment ALL)*(segment ALL) * (market ALL), hh*sum*f=comma12. / nocellmerge;
...
class tran_segment segment band;
var hh;
table (tran_segment ALL)*(segment ALL) * (band ALL), hh*sum*f=comma12. / nocellmerge;
...
class tran_segment segment tenure_yr;
var hh;
table (tran_segment ALL)*(segment ALL) * (tenure_yr ALL), hh*sum*f=comma12. / nocellmerge;
...
class tran_segment segment svcs;
var hh;
table (tran_segment ALL)*(segment ALL) * (svcs ALL), hh*sum*f=comma12. / nocellmerge;
...
class tran_segment segment cqi;
var hh;
table (tran_segment ALL)*(segment ALL) * (cqi ALL), hh*sum*f=comma12. / nocellmerge;
format cqi comma2. segment segfmt. ;
run;
data cqi_results;
set cqi_results;
if substr(_type_,1,1) eq 0 then do;
...
class tran_segment segment cqi_dd;
var hh;
table (tran_segment ALL)*(segment ALL) * (cqi_dd ALL), hh*sum*f=comma12. / nocellmerge;
...
class tran_segment segment cqi_bp;
var hh;
table (tran_segment ALL)*(segment ALL) * (cqi_bp ALL), hh*sum*f=comma12. / nocellmerge;
...
class tran_segment segment cqi_web;
var hh;
table (tran_segment ALL)*(segment ALL) * (cqi_web ALL), hh*sum*f=comma12. / nocellmerge;
...
class tran_segment segment cqi_deb;
var hh;
table (tran_segment ALL)*(segment ALL) * (cqi_deb ALL), hh*sum*f=comma12. / nocellmerge;
...
class tran_segment segment cqi_odl;
var hh;
table (tran_segment ALL)*(segment ALL) * (cqi_odl ALL), hh*sum*f=comma12. / nocellmerge;
...
class tran_segment segment rm;
var hh;
table (tran_segment ALL)*(segment ALL) * (rm ALL), hh*sum*f=comma12. / nocellmerge;
...
edeliv_num = 0;
estat_num = 0;
keep tran_segment segment HHID HH web bp WAP SMS edeliv estat fico FWorks web_signon BP_NUM BP_AMT SMS_NUM WAP_NUM fico_num fworks_num web1 bp1 sms1 wap1 fico1 fworks1 edeliv1 estat1 estat_num edeliv_num;
...
table (tran_segment ALL)*(segment ALL), (sum)*(HH web bp WAP SMS edeliv estat fico FWorks web_num BP_NUM BP_AMT SMS_NUM WAP_NUM fico_num fworks_num web1 bp1 sms1 wap1 fico1 fworks1 edeliv1 estat1 estat_num edeliv_num) / nocellmerge;
run;
data web2;
set web1;
if substr(_type_,1,1) eq 0 then do;
...
keep tran_segment segment service enrolled: active: volume_avg spend_avg;
run;
*************************************************;
...
keep tran_segment segment HHID HH web web1 bp1 bp web_signon BP_NUM BP_AMT vpos: mpos: br_tr: vru: chk: dd: atmo: atmt: cqi_dd ;
...
table (tran_segment ALL)*(segment ALL), (sum)*(HH vpos: mpos: chk: atmo: atmt: dd: bp: ) / nocellmerge;
run;
data tran2;
set tran1;
if substr(_type_,1,1) eq 0 then do;
...
keep tran_segment segment Transaction Enrolled: active: volume_avg spend_avg;
run;
******************************************;
...
set data.main_201203;
where tran_segment ne '';
keep tran_segment segment HHID HH clv: ixi: ;
run;
proc tabulate data=temp_clv out=clv1;
where clv_total ne .;
class tran_segment segment ;
var HH clv:;
table (tran_segment ALL)*(segment ALL), N (clv_total clv_rem clv_rem_ten)*mean;
run;
data clv2;
set clv1;
if substr(_type_,1,1) eq 0 then do;
...
where clv_total ne .;
class tran_segment segment clv_total;
var HH ;
table (tran_segment ALL)*(segment ALL) ,(clv_total ALL)*(hh)*(sum rowpctsum<hh>) ;
...
format segment segfmt.;
keep tran_segment segment clv_total HHs percent_hhs;
run;
proc tabulate data=temp_clv out=ixi1;
where ixi_tot ne .;
class tran_segment segment ;
var HH ixi: ;
table (tran_segment ALL)*(segment ALL), N (ixi:)*mean;
run;
data ixi2;
set ixi1;
if substr(_type_,1,1) eq 0 then do;
...
where ixi_tot ne .;
class tran_segment segment IXI_tot;
var HH ;
table (tran_segment ALL)*(segment ALL) ,(ixi_tot ALL)*(hh)*(sum rowpctsum<hh>) ;
...
percent_hhs = sum (of hh_pctsum:);
format segment segfmt.;
keep tran_segment segment ixi_tot HHs percent_hhs;
run;
ods html;
